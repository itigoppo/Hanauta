...
class login{
	...
	function regist($token){
		...
		$rtn = false;
		$rtn_arr = array("regist"=>false);
		$now_date = gmdate("Y-m-d H:i:s",time() + $Hanauta->site_info["time_zone"]);

		// エラーチェック
		$chk_arr = array(
				"login_id" => array(
								array("rule"=>"len","msg"=>mb_ereg_replace("##MSG##","ID",$Hanauta->error["E0012"]),"param"=>array("max"=>20,"min"=>3)),
								array("rule"=>"han_mix","msg"=>mb_ereg_replace("##MSG##","ID",$Hanauta->error["E0007"])),
								array("rule"=>"noempty","msg"=>mb_ereg_replace("##MSG##","ID",$Hanauta->error["E0001"])),
				),
				"login_pass" => array(
								array("rule"=>"len","msg"=>mb_ereg_replace("##MSG##","パスワード",$Hanauta->error["E0012"]),"param"=>array("max"=>20,"min"=>3)),
								array("rule"=>"han_mix","msg"=>mb_ereg_replace("##MSG##","パスワード",$Hanauta->error["E0007"])),
								array("rule"=>"noempty","msg"=>mb_ereg_replace("##MSG##","パスワード",$Hanauta->error["E0001"])),
				),
				"keyword" => array(
								array("rule"=>"same","msg"=>$Hanauta->obj["twitter"]->regist_ng,"param"=>array("word"=>$Hanauta->obj["twitter"]->regist_word)),
								array("rule"=>"noempty","msg"=>mb_ereg_replace("##MSG##","キーワード",$Hanauta->error["E0001"])),
				),
		);
		...
		if(!$error["error"]){
			...
			$sql = "insert into ".$tbl_name."(id,pass,salt,token,token_secret,ip,regist_date) values("
					. "'".$Hanauta->_pvars["login_id"]."',"
					. "'".$Hanauta->_pvars["login_pass"]."',"
					. "'".$rand_key."',"
					. "'".$token["access_token"]."',"
					. "'".$token["access_token_secret"]."',"
					. "'".$Hanauta->_srvars["REMOTE_ADDR"]."',"
					. "'".$now_date."'"
					. ")";
			$result = $Hanauta->obj["read_db"]->send_query($sql,1);
			if($result) $rtn_arr["regist"] = true;
		}
